...
An aggregate is an instance of a class (or more properly, a record) that handles one or more models. We will be able to recreate the state of the aggregate by processing the stored events by the  _evolve_ function. As we will see later, the _events_ are closely related to some of the members of the aggregate. 
...
Note that in the _in memory_ or _Postgres_ storage implementations the state of any aggregate is rebuilt starting from the last available snapshot and applying the events that are after the snapshot.
This strategy is slightly different in EventStore and CosmosDb implementations, and I am still working on it in separate branches.
...
It uses the "result" computational expression included in the FsToolkit.ErrorHandling library which supports the [_railway-oriented programming_](https://fsharpforfunandprofit.com/rop/) style of handling errors.
